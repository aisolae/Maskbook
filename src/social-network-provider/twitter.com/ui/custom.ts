...
export function useTheme() {
    ...
    useEffect(() => {
        const MaskbookTheme = isDark(backgroundColor) ? MaskbookDarkTheme : MaskbookLightTheme
        setTheme(
            createMuiTheme({
                ...MaskbookTheme,
                palette: {
...
function useBackgroundColor() {
    ...
    useLayoutEffect(() => {
        const timer = setInterval(() => {
            ...
            if (color) {
                setBackgroundColor(toRGB(color))
            }
        }, 2000)
        return () => clearInterval(timer)
    }, [backgroundColor])
...
function isDark(color: string) {
    ...
    return parseInt(r) < 68 && parseInt(g) < 68 && parseInt(b) < 68
}
